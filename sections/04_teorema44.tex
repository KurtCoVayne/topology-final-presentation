% ============================================================
\section{Teorema 4.4 — Espacios simplemente conexos}
% ============================================================

\begin{frame}{Teorema 4.4 — Espacio simplemente conexo}
...
\begin{frame}{Teorema 4.4 — Espacio contráctil}
...
\begin{frame}{Teorema 4.4 — Enunciado}
...
\begin{frame}{Demostración — Parte 2: Caso $H(\xo, s) = \xo$}
  ...
  \[
    \boxed{h(t,\, s) \;=\; H\!\bigl(\alpha(t),\, s\bigr)}
  \]
...
\begin{frame}{Demostración — Parte 2: Conclusión}
  ...
  \begin{block}{Conclusión}
    $h$ es una homotopía de lazos de $\alpha$ al lazo constante $c$.
    Por tanto $\clase{\alpha} = \clase{c}$, y $\pione(X, \xo)$ contiene
    únicamente el elemento identidad.

    \medskip
    Luego $X$ es \textbf{simplemente conexo}. \hfill $\checkmark$
  \end{block}
...
\begin{frame}{Demostración — Parte 3: Caso general}
  ...
  $\xo$ hasta el nivel $s$.  La homotopía corregida es:
  \[
    \boxed{\tilde{h}(\cdot,\, s) \;=\; \beta_s * h(\cdot,\, s) * \bar{\beta}_s.}
  \]
  ...
  \bigskip
  \begin{block}{Conclusión}
    En ambos casos, $\clase{\alpha} = \clase{c}$ para todo lazo $\alpha$
    en $\xo$, luego $\pione(X,\xo) = \{\clase{c}\}$: el grupo trivial.
    \hfill $\square$
  \end{block}

\end{frame}

\begin{frame}{Teorema 4.4 — Ilustración (Figura 4.5)}
  ...
  \begin{tikzpicture}[scale=1.1]
    ...
    % Punto base x0
    \filldraw[morado] (0,-0.8) circle (2pt) node[below, morado]
      {\small $\xo$};

    % Camino H(x0, .) desde x0  (su restriccion a [0,s] es beta_s)
    \draw[morado, thick, ->, >=Stealth]
      (0,-0.8) .. controls (-0.3,0.2) and (-0.8,0.8) .. (-0.6,1.4);
    \node[morado] at (-1.25,0.0) {\small $H(\xo,\cdot)$};
    \node[morado] at (0.02,0.45) {\small $\beta_s$};

    % Punto H(x0, s) en el nivel intermedio
    \filldraw[morado!70] (-0.6,1.4) circle (2pt);
    \node[morado!80, left=3pt] at (-0.6,1.4) {\small $H(\xo,s)$};

    % Lazo h(·,s) en el nivel s
    \draw[coralOscuro, thick]
      (-0.6,1.4) .. controls (0.4,2.1) and (1.5,1.7) ..
      (1.0,1.0) .. controls (0.6,0.4) and (0.0,0.8) .. (-0.6,1.4);
    \node[coralOscuro] at (1.15,1.75) {\small $h(\cdot,s)$};

    \node at (2.1,1.7) {\small $X$};
  \end{tikzpicture}
  \end{center}

  \vspace{-2mm}
  \begin{itemize}
    \item El camino $H(\xo,\cdot)$ (\textcolor{morado}{azul}) puede no
          ser constante.
    \item El lazo corregido $\tilde{h}(\cdot,s)$ recorre $\beta_s$,
          el nivel $h(\cdot,s)$ (\textcolor{coralOscuro}{rojo}),
          y regresa por $\bar{\beta}_s$.
  \end{itemize}

\end{frame}

\begin{frame}{Teorema 4.4 — Resumen}
  ...
  \begin{enumerate}
    ...
    \item En ambos casos $\pione(X,\xo) = \{\clase{c}\}$: el grupo trivial.
  \end{enumerate}
